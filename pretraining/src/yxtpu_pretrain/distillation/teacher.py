...
class Qwen35Teacher:
    ...
    def score(self, student_input_ids, positions, segment_ids):
        ...
        if self._scorer is None:
            ...
            #
            # A split-once jax.jit variant (split at construction, merge
            # inside the trace) was tried to shave nnx.jit's per-call graph
            # walk and REVERTED: under it the layer scan carried the whole
            # parameter set UNSHARDED - a batch-independent 21.49G program
            # reservation against a 30.75G chip, the a68adaa signature -
            # where nnx.jit keeps the scan carry sharded. The per-call walk
            # is paid knowingly.
