...
"""The three-pass (bf16x3) production solve of the folded kernel must stay
within its qualified error envelope on the correlated-key fixture, and the
six-pass rollback path must remain intact. The bf16 splits are explicit, so
CPU arithmetic emulates the TPU pass structure faithfully (exact products of
rounded operands with fp32 accumulation)."""

import jax
import jax.numpy as jnp
import numpy as np

from yxtpu_pretrain.kernels import kda_fused_pallas as kernel
