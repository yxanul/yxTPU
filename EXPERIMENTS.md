...
| EXP-021 | Replace the doubling solve with blocked forward substitution to make it BF16-safe | Rejected on both counts. The expected fivefold arithmetic saving yields 1.6% (6.315 vs 6.418 ms), so the solve is bound by short 16-row matmuls and four serial steps rather than FLOPs; and confining the series per block still reaches NaN in BF16, at step zero. Retained as a control | `results/v6e8-kda-substitution-bf16-20260720/` |
